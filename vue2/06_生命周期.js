 ...
 * 4、运行生命周期钩子函数 cteated
 * 5、生成 render 函数，如果有配置，直接使用配置的 render，如果没有，使用运行时编译器，把模版编译成 render
 ...
 * 在执行 _update 函数的过程中，触发 patch 函数(diff 对比)，由于目前没有旧树，因此直接为当前的虚拟 dom 树的每一个普通节点生成 elm 属性，即真实 dom
 ...
 * 2、Watcher 会被调度器放到 nextTick 中运行，也就是微队列，这样是为了避免多个依赖的数据同时改变后被多次执行
 * 3、运行生命周期钩子函数 beforeUpdate
 * 4、updateComponent 函数重新执行
 * 在执行 render 函数的过程中，会去掉之前的依赖，重新收集所有依赖，将来依赖变化时会重新运行 updateComponent 函数
 * 在执行 _update 函数的过程中，触发 patch 函数，新旧两棵树进行对比，普通 html 节点的对比会导致真实节点被创建、删除、移动、更新
 * 当新组件需要创建时，进入实例化流程
